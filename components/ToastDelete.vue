<template>
  <div v-if="isModalVisible" class="fixed inset-0 z-40 min-h-full overflow-y-auto overflow-x-hidden transition flex items-center">
    <!-- overlay -->
    <div aria-hidden="true" class="fixed inset-0 w-full h-full bg-black/60 cursor-pointer" @click="closeModal"></div>

    <!-- Modal -->
    <div class="relative w-full cursor-pointer pointer-events-none transition my-auto p-4">
      <div class="w-full py-2 bg-white cursor-default pointer-events-auto  relative rounded-xl mx-auto max-w-sm">
        <button tabindex="-1" type="button" class="absolute top-2 right-2 rtl:right-auto rtl:left-2" @click="closeModal">
...
export default {
  setup() {
    const isModalVisible = useState('modalVisible', () => false); // Modal visibility state
